...
特性（Attribute）是用于在运行时传递程序中各种元素（比如类、方法、结构、枚举、组件等）的行为信息的声明性标签。程序员可以将特性附加到各种程序实体，并在运行时环境中检索特性信息。
...
特性用于添加元数据，如编译器指令和注释、描述、方法、类等其他信息。应用了特性的程序结构被称为**目标**，用来获取或使用元数据的程序被称为**消费者**。
...
`System.Runtime.CompilerServices`提供了三个用于向可选参数提供有关调用上下文的信息的特性：`CallerLineNumberAttribute`、`CallerFilePathAttribute`和`CallerMemberNameAttribute`。在方法形参之前添加调用者信息特性可以获取相应的调用者信息。被调用者信息特性批注的参数需要赋初值。
...
自定义特性类命名时应使用Pascal命名法，必须以`Attribute`后缀结尾，必须继承`Attribute`类。特性类应该表示目标结构的一些状态。如果特性需要字段，可以包含具有位置参数（按构造函数中参数的位置赋值）的构造函数来收集数据，可选字段可以采用命名参数（按照属性的名字进行初始化）按需初始化。除了属性之外，不要使用公共方法和其他方法成员。为了安全，把特性类声明为`sealed`（不能被继承）。在特性类使用`AttributeUsage`特性指定可应用到的目标组。
...
internal sealed class InfomationAttribute : Attribute
{
    ...
    public InfomationAttribute (string description)
    {
...
``` C#
[Infomation("这是一个无用类", Developer = "ZhengXing", Version = "v0.1")]
class Program
{
    static void Main ()
    {
        ...
        bool rel = t.IsDefined(typeof(InfomationAttribute), flase); // 检查此类是否某应用特性
