...
值类型进一步分为简单类型（布尔`bool`；字符`char`；整型`byte`，`sbyte`，`ushort`，`short`，`uint`，`int`，`ulong`，`long`；浮点`float`，`double`，`decimal`）、枚举`enum`、结构体`struct`、元组、可为null值类型。**所有简单类型都是`struct`类型**；可以使用文本为简单类型初始化，可以用`const`定义声明简单类型的常量。
...
`bool`转换为字符串时为"True"或"False"；可将"True"或"False"的字符串（不区分大小写，前面和后面可以有空格）转换为`bool`。非零值整型或浮点数转换将转换为`true`，零值则转换为`false`；`bool`转换为数值时，`true`转换为1，`false`则转换为0。

在比较大小时，`true` > `false` > `null`。

### 字符

字符类型`char`（`System.Char`）表示Unicode UTF-16字符，默认值为`\0`，2字节，范围为0x0000到0xFFFF。

每⼀个字符都对应⼀个整数，即Unicode字符编码。可用字符文本、Unicode转义序列（`\u`后跟字符代码的十六进制表示形式，四个符号）、十六进制转义序列（`\x`后跟字符代码的十六进制表示形式，四个符号）和整数（每个Unicode字符编码的值）为`char`指定值，除整数外其他形式需要被单引号`'`包裹。
...
除了使用`char`表示单个字符外，UTF-16编码还允许使用两个`char`表示抽象字符，这被称为**代理项对**（Surrogate Pair）。其中前一个被称作高代理项（High Surrogate），代码点的范围为0xD800到0xDBFF；后一个被称作低代理项（Low Surrogate），代码点的范围为0xDC00到0xDFFF。单独的代理项没有意义，它只有在作为代理对的一部分使用时才有意义。

`char`支持比较运算符。对于算术和位运算符，会对相应的字符编码执行操作，并得出`int`结果。
...
| 无符号指针整型 | `UIntPter` | - | 4或8字节 | 根据系统而定 |
| 有符号指针整型 | `IntPter` | - | 4或8字节 | 根据系统而定 |
...
| 名称 | 关键字 | 后缀 | 内存占用 | 储值范围 |
| --- | --- | --- | --- | --- |
| 单精度浮点数 | `float` | f、F | 4字节 | -3.4E+38 ~ 3.4E+38（6-7位有效数字） |
| 双精度浮点数 | `double` | d、D | 8字节 | -1.8E+308 ~ 1.8E+308（15位有效数字） |
| 高精度浮点数 | `decimal` | m、M | 16字节 | -2^96^-1 ~ 2^96^-1（28-29位有效数字） |
