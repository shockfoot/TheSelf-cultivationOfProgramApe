...
Clear Flags清除标识：决定屏幕的空白部分如何处理。Skybox天空盒、Solid Color纯色、Depth Only仅深度（仅渲染游戏物体）、Don't Clear不清除。

## 摄像机属性

Background背景颜色。

Culling Mask剔除遮罩，选择摄像机渲染的层。

Projection模式：Perspective透视、Orthographic正交。

Field of View透视摄像机的视口范围。

Clipping Planes透视摄像机渲染远近点。

Size正交摄像机的视口范围。

Viewport Rect视口矩形：标明摄像机视图在屏幕上绘制的屏幕坐标。X、Y水平和垂直开始位置，W、H宽高。

Depth深度：多个摄像机的渲染顺序，越大越后渲染。

Rendering渲染路径：Use Graphics Settings使用绘图设置、Forward快速渲染（所以对象按照每种材质一个通道的方式渲染）、Deferred延迟照明（所以对象将无照明绘制一次，然后所有对象的照明将一起在渲染队列的末尾被渲染）、Legacy Vertex Lit顶点光照（对所有对象的渲染会作为顶点光照对象渲染）、Legacy Deferred（light prepass）废弃的延迟光照。

Target Texture渲染纹理。

Occlusion Culling遮挡剔除。

Allow HDR高动态范围图像：提供更多的动态范围和图像细节。
